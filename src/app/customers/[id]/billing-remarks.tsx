...
export default function BillingRemarksScreen() {
  const params = useLocalSearchParams<{ id?: string }>();
  const customer = getCustomerById(params.id ?? '');

  if (!customer) return <MissingCustomer />;

  return <BillingRemarksScreenContent customer={customer} />;
}

function BillingRemarksScreenContent({ customer }: { customer: CustomerRecord }) {
  const { content, footer } = useBillingRemarksPanel(customer);

  return (
    <Screen scroll edges={['bottom']} contentStyle={styles.screen} bottomAccessory={footer}>
      <CustomerSectionHeader title="JMR / Billing Remarks" customer={customer} />
      {content}
    </Screen>
  );
}
