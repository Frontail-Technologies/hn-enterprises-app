...
export type AssignedProject = {
  ...
  targets: {
    ...
    plannedSurveys: string;
    plannedPlumbingGi: string;
    plannedGc: string;
...
export const notifications: Notification[] = [
  {
    id: 'noti-001',
    title: 'GC evidence sent back',
    message: 'Meena Sharma requires clearer GC evidence before approval.',
    category: 'Work',
    createdAt: '2026-07-21T09:10:00.000Z',
    read: false,
    route: { pathname: '/work/[id]', params: { id: 'wp-002' } },
  },
  {
    id: 'noti-002',
    title: 'Attendance pending',
    message: 'Please mark attendance with current location before starting site work.',
    category: 'Attendance',
    createdAt: '2026-07-21T08:35:00.000Z',
    read: false,
  ...
  },
  {
    ...
    message: 'Rajesh Kumar survey is approved and ready for installation planning.',
    category: 'Survey',
    createdAt: '2026-07-20T16:30:00.000Z',
    read: true,
    route: { pathname: '/customers/[id]/survey', params: { id: 'cust-001' } },
  },
  {
    id: 'noti-004',
    title: 'Work queue updated',
    message: '3 assigned work records were refreshed for Shyam Nagar CGD Project.',
    category: 'System',
  ...
  },
  {
    ...
    message: 'Green Mart Store conversion record has been marked completed.',
    category: 'Work',
    createdAt: '2026-07-19T12:00:00.000Z',
    read: true,
    route: { pathname: '/customers/[id]/conversion', params: { id: 'cust-003' } },
  },
  {
    id: 'noti-006',
    title: 'DPR delayed',
    message: 'GC correction visit DPR is delayed due to customer unavailability.',
    category: 'Work',
  ...
  },
  {
    ...
    message: 'Meter photo for Rajesh Kumar was uploaded successfully.',
    category: 'System',
    createdAt: '2026-07-17T10:00:00.000Z',
    read: true,
    route: { pathname: '/customers/[id]/documents', params: { id: 'cust-001' } },
  },
  {
    id: 'noti-008',
    title: 'Survey revision',
    message: 'Meena Sharma survey has a sent-back revision note.',
    category: 'Survey',
    createdAt: '2026-07-16T15:05:00.000Z',
    read: true,
    route: { pathname: '/customers/[id]/survey', params: { id: 'cust-002' } },
  },
];
...
export function getAssignedProjects(): AssignedProject[] {
  const projectNames = Array.from(new Set(customers.map((customer) => customer.projectName)));

  return projectNames.map((projectName) => {
    const projectCustomers = getProjectCustomers(projectName);
    const projectWork = getProjectWorkRecords(projectName);
    const sites = Array.from(new Set(projectCustomers.map((customer) => customer.siteArea)));
    const workDates = projectWork.map((record) => record.lastUpdated).sort();
    const customerDates = projectCustomers.map((customer) => customer.createdDate).sort();
    const lastActivity = workDates[workDates.length - 1] ?? customerDates[customerDates.length - 1] ?? '-';

    const details = getProjectDetails(projectName, projectCustomers[0]?.city ?? '-');
    ...
    return {
      ...
      customersCount: projectCustomers.length,
      pendingWorkCount: projectWork.filter((record) => record.status !== 'Completed').length,
      evidenceCount: projectWork.reduce((total, record) => total + record.evidenceCount, 0),
      lastActivity,
      sites,
    };
  });
...
function getProjectDetails(projectName: string, city: string) {
  if (projectName === 'Shyam Nagar CGD Project') {
    return {
      ...
      targets: {
        plannedCustomers: '5,200',
        plannedSurveys: '5,800',
        plannedPlumbingGi: '4,750',
        plannedGc: '4,200',
        plannedCommissioning: '3,850',
        plannedConversion: '3,600',
        plannedJmr: '42',
      },
      pipeTargets: {
        pipe20Mm: '18.4 KM',
      ...
      siteRecords: [
        {
          id: 'site-sn-a',
          siteName: 'Shyam Nagar Block A',
          siteCode: 'SN-A',
          city: 'Jaipur',
          address: 'Shyam Nagar Block A, Jaipur, Rajasthan',
        ...
        {
          id: 'site-sn-b',
          siteName: 'Shyam Nagar Block B',
          siteCode: 'SN-B',
          city: 'Jaipur',
          address: 'New Sanganer Road, Shyam Nagar, Jaipur',
        ...
        {
          id: 'site-sn-mtr',
          siteName: 'Metro Station Stretch',
          siteCode: 'SN-MTR',
          city: 'Jaipur',
          address: 'Metro Corridor, Shyam Nagar, Jaipur',
  ...
  return {
    ...
    projectType: 'CGD Network',
    city,
    areaLocation: projectName.replace(' CGD Project', '').replace(' Phase 1', ''),
    startDate: '2025-01-15',
    endDate: '2026-01-14',
    contractValue: '₹ 12.50 Cr',
    projectManager: 'Amit Sharma',
    description: `${projectName} covering PNG customer connections and MDPE pipeline network.`,
    contractId: projectName.toUpperCase().replace(/[^A-Z0-9]+/g, '-').replace(/(^-|-$)/g, ''),
    plannedEndDate: '2026-01-14',
    billingMethod: 'Milestone Based',
    targets: {
      plannedCustomers: '5,200',
    ...
    siteRecords: [
      {
        id: `${projectName.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-site-1`,
        siteName: projectName.replace(' CGD Project', ''),
        siteCode: 'SITE-01',
        city,
        address: `${projectName}, ${city}`,
        plannedConnections: 1200,
        supervisor: 'Amit Rathore',
        status: 'Active' as const,
      },
    ],
...
export function getProjectById(projectId: string) {
  return getAssignedProjects().find((project) => project.id === projectId) ?? null;
}

export function getProjectCustomers(projectName: string) {
  return customers.filter((customer) => customer.projectName === projectName);
}

export function getProjectWorkRecords(projectName: string) {
  return workProgressRecords.filter((record) => record.projectName === projectName);
}
