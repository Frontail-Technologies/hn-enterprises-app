...
type DateFieldProps = Omit<TextInputProps, 'placeholderTextColor' | 'onChange'> & {
  label: string;
  value?: string;
...
export function DateField({ label, value, editable = true, onChangeText }: DateFieldProps) {
  ...
  return (
    <View style={styles.wrapper}>
      ...
      >
        ...
        <CalendarDays size={18} color={editable ? colors.primary : colors.muted} />
      </Pressable>
          ...
          <View style={styles.daysGrid}>
            {days.map((day, index) => {
              ...
              return (
                ...
                >
                  <Text style={[styles.dayText, { color: active ? '#FFFFFF' : colors.text }]}>
                    {day ? day.getDate() : ''}
                  </Text>
                </Pressable>
              );
