...
export function useMdpeFittingsPanel(
  ...
  const { values, updateField, clearDraft, draftState, loadingDraft, isDirty } =
    useDraftForm(
  ...
  const submit = async () => {
    ...
    try {
      ...
      console.error("[MdpeFittingsPanel] submit failed", {
  ...
  const footer = (
    <SectionFormFooter
      onSubmit={submit}
      isSubmitting={updateMdpeFittingsMutation.isPending}
      disabled={!isDirty}
    />
  );
